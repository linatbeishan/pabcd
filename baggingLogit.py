...
'''
baggingLogit.py
Desc :  Bagging Logit algorithm used to predict circRNA-disease associations.
Usage: ./baggingLogit.py directory-contains-feature-files outDir NumBootstrap randomSeed
E.g. : ./baggingLogit.py ./features ./baggingLogit-outdir NumBootstrap randomSeed
Coder: linwei, etc
...
#    fif = "/".join((outdir, featImp))
    ...
    for ikf in range(nfolds):
            ...
            for r in R:
                ...
                for ikf2 in range(nfolds):
                    ...
                    for i in range(T):
                        ...
                        clf = LogisticRegression(penalty = "l2", C = c, class_weight = { -1: 1, 1: r}, random_state = i)
        ...
        for i in range(T):
            ...
            clf = LogisticRegression(penalty = "l2", C = optimalC, class_weight = { -1: 1, 1: optimalR}, random_state = i)
...
#                output.write("%s-%dth fold-%dth classifier, feature importance:%s\n" %(dId, ikf, i, coefStr))
            scores = clf.decision_function(X_pu_test_transformed)
